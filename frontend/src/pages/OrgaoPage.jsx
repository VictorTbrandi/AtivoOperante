...
export default function OrgaoPage() {
  ...
  const salvarTipo = () => {
    ...
    const url = isEditando ? `${API_BASE_URL}/orgao/${tipoAtual.id}` : `${API_BASE_URL}/orgao`;
  ...
  const confirmarExclusao = () => {
    fetch(`${API_BASE_URL}/orgao/${tipoParaExcluir.id}`, {
      method: 'DELETE',
      headers: {
        'Authorization': `Bearer ${localStorage.getItem("token")}`
      }
  ...
  return (
    <div className="container mt-4">
      <h2>Tipos de Órgãos</h2>
      <Button className="mb-3" onClick={abrirModalNovo}>Novo Órgao</Button>
      ...
      <Modal show={showModal} onHide={() => setShowModal(false)}>
        <Modal.Header closeButton>
          <Modal.Title>{isEditando ? 'Editar Tipo' : 'Novo Órgão'}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Form.Group>
